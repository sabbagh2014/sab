...
Mongoose.model("referral", referralSchema).find({ status: { $ne: status.DELETE } }, async (err, result) => {
    if (err) {
        ...
        Mongoose.model("referral", referralSchema).create(obj, (err1, staticResult) => {
            if (err1) {
