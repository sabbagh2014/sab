...
Mongoose.model("social", schemaDefination).find({ status: { $ne: status.DELETE } }, async (err, result) => {
    if (err) {
        ...
        Mongoose.model("social", schemaDefination).create(obj1, obj2, obj3, obj4, obj5, (err1, staticResult) => {
            if (err1) {
