...
const nftMinting = async (fromAddress, privateKey, walletAddress, nftDetails) => {
    try {
        const { tokenName, mediaUrl } = nftDetails
        const { gasPrice } = await EthHelper()
        const Data = await myNFTContract.methods.create(walletAddress, mediaUrl, tokenName).encodeABI();
