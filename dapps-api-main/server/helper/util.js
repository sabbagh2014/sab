...
const { createFee, findFee, updateFee, feeList, sortFee } = feeServices;
...
module.exports = {
  ...
  async getFeeCategory(blockchainUrl, address) {
    let getCategory;
    let massBalance = await axios.get(`${blockchainUrl}/getBalance/${address}/MASS`);
    getCategory = await sortFee({ massHeld: { $lte: massBalance.data.balance } });
    if (!getCategory) {
  ...
  sendMail: async (to, name, link, type) => {
    ...
    <div style="margin:50px auto;width:70%;padding:20px 0">
      ...
      <div align="center">
        <a href="${type == "admin" ? config.get('hostAddress') : config.get('userResetLink')}?token=${link}" target="_blank" style="box-sizing: border-box;display: inline-block;font-family:'Montserrat',sans-serif;text-decoration: none;-webkit-text-size-adjust: none;text-align: center;color: #FFFFFF; background-color: #0088ee; border-radius: 60px;-webkit-border-radius: 60px; -moz-border-radius: 60px; width:auto; max-width:100%; overflow-wrap: break-word; word-break: break-word; word-wrap:break-word; mso-border-alt: none;border-top-color: #CCC; border-top-style: solid; border-top-width: 0px; border-left-color: #CCC; border-left-style: solid; border-left-width: 0px; border-right-color: #CCC; border-right-style: solid; border-right-width: 0px; border-bottom-color: #0275a4; border-bottom-style: solid; border-bottom-width: 5px;">
