...
const reportServices = {
    ...
    findReport: async (query) => {
        return await reportModel.findOne(query).populate([{ path: 'userId', select: '-ethAccount.privateKey -password -referralCode -email -permissions' }, { path: 'chatId' }]);
    },
    ...
    reportList: async (query, page, limit) => {
        ...
        var options = {
            ...
            sort: { createdAt: -1 },
            populate: [{ path: 'userId', select: '-ethAccount.privateKey -password -referralCode -email -permissions' }, { path: 'chatId' }],
        };
