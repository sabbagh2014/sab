...
const nftServices = {
    ...
    findNft: async (query) => {
        return await nftModel.findOne(query).populate('userId').select('-ethAccount.privateKey -password -referralCode -email -permissions');
    },
    ...
    listAllNft: async (validatedBody) => {
        ...
        return await nftModel.find(query).populate({ path: 'userId', select: '-ethAccount.privateKey' }).sort({ createdAt: -1 });
    },

    nftListWithAggregatePipeline: async (validatedBody, userId) => {
        let query = { userId: mongoose.Types.ObjectId(userId), status: { $ne: status.DELETE } };
        const { search } = validatedBody;
        if (search) {
            query.$or = [
                { tokenId: { $regex: search, $options: 'i' } },
    ...
    nftListWithAggregatePipelineForAll: async (validatedBody, userId) => {
        let activeIds = await getActiveUser();
        let query = { userId: userId, userId: { $in: activeIds }, status: { $ne: status.DELETE } };
        const { search } = validatedBody;
        if (search) {
            query.$or = [
                { tokenId: { $regex: search, $options: 'i' } },
    ...
    sharedBundleListPerticular: async (userId, bundleIds) => {
        if (userId) {
            return await nftModel.aggregate([
                { $match: { _id: mongoose.Types.ObjectId(bundleIds), status: { $ne: status.DELETE } } },
                {
                    $addFields: {
                        "isSubscribe": {
                            $cond: {
