...
const transactionServices = {
    ...
    transactionList: async (query) => {
        ...
        return await transactionModel.find(query).populate([{ path: 'userId', select: '-ethAccount.privateKey -password -referralCode -email -permissions', populate: { path: 'subscribeNft' } }, { path: 'nftId' }, { path: 'nftUserId', select: '-ethAccount.privateKey' }]);
    },
    ...
    depositListWithPagination: async (userId, validatedBody) => {
        ...
        var query = { userId: userId, status: status.ACTIVE, transactionType: "Donation" }
        var options = {
            page: parseInt(page) || 1,
            limit: parseInt(limit) || 10,
            sort: { createdAt: -1 },
            populate: { path: 'toDonationUser', select: 'name userName email ethAccount.address' }
        };
    ...
    depositListWithPopulate: async (userId, validatedBody) => {
        const { page, limit } = validatedBody
        var query = { $and: [{ status: status.ACTIVE }, { transactionType: { $ne: "Deposite" } }, { $or: [{ userId: userId }, { toDonationUser: userId }, { nftUserId: userId }] }] }
        var options = {
            page: parseInt(page) || 1,
            limit: parseInt(limit) || 10,
            sort: { createdAt: -1 },
            populate: [{ path: 'userId', select: '-ethAccount.privateKey -password -referralCode -email -permissions' }, { path: 'toDonationUser' }, { path: 'nftUserId' }]
        };
        return await transactionModel.paginate(query, options);
    },
    ...
    allTransactions: async (validatedBody) => {
        ...
        var query = { $and: [{ status: status.ACTIVE }, { $or: [{ userId: userId }, { toDonationUser: userId }, { nftUserId: userId }] }] }
        if (fromDate && !toDate) {
