...
const orderServices = {
    ...
    orderList: async (query) => {
        ...
        return await orderModel.find(query).populate([{ path: 'userId', select: '-ethAccount.privateKey -password -referralCode -email -permissions' }, { path: 'nftId' }]);
    },

    soldOrderList: async (query) => {
        // let activeIds = await getActiveUser();
        // query.userId = { $in: activeIds };
        return await orderModel.find(query).populate([{ path: 'userId', select: '-ethAccount.privateKey -password -referralCode -email -permissions' }, { path: 'buyerId', select: '-ethAccount.privateKey -password -referralCode -email -permissions' }, { path: 'sellerId', select: '-ethAccount.privateKey' }, { path: 'nftId' }]);
    },
