...
const userServices = {
  ...
  findUserWithSelect: async (query) => {
    return await userModel.findOne(query).select('-ethAccount.privateKey -password -permissions -otp');
  },
  ...
  userSubscriberList: async (query) => {
    return await userModel.find(query).select('-ethAccount.privateKey -password -permissions -otp');
  },
  ...
  latestUserListWithPagination: async (search, page, limit, type) => {
    ...
    var options = {
      ...
      limit: parseInt(limit) || 10,
      sort: { createdAt: -1 },
      select: '-ethAccount.privateKey -password -email -referralCode -permissions -otp'
    };
    return await userModel.paginate(query, options);
  },
  ...
  allUsersList: async (userId, validatedBody) => {
    ...
    let query = { _id: { $ne: userId }, status: status.ACTIVE, userType: { $in: [userType.USER, userType.CREATOR] } };
    if (type) {
      query.userType = type;
    }
    if (search) {
  ...
  updateUser: async (query, updateObj) => {
    return await userModel.findOneAndUpdate(query, updateObj, { new: true }).select('-ethAccount.privateKey -password -otp');
  },

  updateUserById: async (query, updateObj) => {
    return await userModel.findByIdAndUpdate(query, updateObj, { new: true }).select('-ethAccount.privateKey -password -otp');
  },
  ...
  paginateSearch: async (validatedBody) => {
    ...
    let options = {
      ...
      limit: limit || 15,
      sort: { createdAt: -1 },
      select: '-ethAccount.privateKey -password -referralCode -permissions -otp'
    };
    return await userModel.paginate(query, options);
  },

  profileSubscriberList: async (subscriberList) => {
    return userModel.find({ _id: { $in: subscriberList }, status: { $ne: status.DELETE } }).select('-ethAccount.privateKey -profileSubscribe -subscriberList -permissions -otp');
  },

  profileSubscribeList: async (profileSubscribe) => {
    return userModel.find({ _id: { $in: profileSubscribe }, status: { $ne: status.DELETE } }).select('-ethAccount.privateKey -profileSubscribe -subscriberList -permissions -otp');
  },
  ...
  userList: async (validatedBody) => {
    let query = { status: { $ne: status.DELETE }, userType: { $in: [userType.USER, userType.CREATOR] } };
    const { search, type, planType, fromDate, toDate, page, limit } = validatedBody;
    if (search) {
      query.$or = [
        { name: { $regex: search, $options: 'i' } },
        { email: { $regex: search, $options: 'i' } },
  ...
  adminModeratorList: async (validatedBody) => {
    ...
    let options = {
      ...
      limit: limit || 10,
      sort: { createdAt: -1 },
      select: '-ethAccount.privateKey -password -referralCode -email -permissions -otp'
    };
    return await userModel.paginate(query, options);
  },
  ...
  listSubAdmin: async (validatedBody) => {
    ...
    var query = { status: { $ne: status.DELETE }, userType: userType.SUB_ADMIN };
    if (search) {
      query.$or = [
        { name: { $regex: search, $options: 'i' } },
        { email: { $regex: search, $options: 'i' } },
