...
const auctionNftServices = {
    ...
    findAuctionNft: async (query) => {
        return await auctionNftModel.findOne(query).populate([{ path: 'userId', select: '-ethAccount.privateKey -password -referralCode -email -permissions' }, { path: 'orderId' }, { path: 'nftId' }]);
    },
    ...
    auctionNftList: async (query) => {
        ...
        return await auctionNftModel.find(query).populate([{ path: 'userId', select: '-ethAccount.privateKey -password -referralCode -email -permissions' }, { path: 'orderId' }, { path: 'nftId' }]).sort({ createdAt: -1 });
    },
    ...
    allNftAuctionList: async (validatedBody) => {
        ...
        let options = {
            ...
            limit: limit || 10,
            sort: { createdAt: -1 },
            populate: [{ path: 'userId', select: '-ethAccount.privateKey -password -referralCode -email -permissions' }]
        };
