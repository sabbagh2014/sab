...
const { userCheck, checkUserExists, emailMobileExist, createUser, findUser, updateUser, updateUserById, checkSocialLogin, userCount, userList } = userServices;
...
export class planController {
    ...
    async choosePlan(req, res, next) {
        ...
        try {
            ...
            let subscriptionCheck = await findSubscription({ _id: validatedBody.subscriptionId, orderPlace: false, status: { $ne: status.DELETE } });
    ...
    async viewPlan(req, res, next) {
        ...
        try {
            ...
            var planResult = await findPlan({ _id: _id, status: { $ne: status.DELETE } });
            if (!planResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(planResult, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
        }
    ...
    async editPlan(req, res, next) {
        ...
        try {
            ...
            var planResult = await findPlan({ _id: validatedBody._id, status: { $ne: status.DELETE } });
            if (!planResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
    ...
    async deletePlan(req, res, next) {
        ...
        try {
            ...
            var result = await updatePlan({ _id: planResult._id }, { status: status.DELETE });
    ...
    async listPlan(req, res, next) {
        try {
            ...
            let dataResults = await planList({ userId: userResult._id, status: { $ne: status.DELETE } });
