...
const { createTransaction, findTransaction, updateTransaction, transactionList, depositeList } = transactionServices;

const { userCheck, updateUser, findUser, findUserData } = userServices;
...
const { createOrder, findOrder, findOrderWithPopulate, updateOrder, orderList, soldOrderList } = orderServices;
...
export class orderController {
    ...
    async createOrder(req, res, next) {
        ...
        try {
            ...
            let nftCheck = await findAuctionNft({ _id: validatedBody.nftId, status: { $ne: status.DELETE } });
            if (!nftCheck) {
            ...
            await updateAuctionNft({ _id: nftCheck._id }, { orderPlace: true, time: validatedBody.time }, { $inc: { treandingNftCount: 1 } });
            let obj = {
    ...
    async editOrder(req, res, next) {
        ...
        try {
            ...
            await updateAuctionNft({ _id: result.nftId }, { orderPlace: true, time: validatedBody.time, auctionStatus: "RUNNING", isSold: false, isBuy: false }, { $inc: { treandingNftCount: 1 } });
    ...
    async listOrder(req, res, next) {
        try {
            ...
            let dataResults = await orderList({ userId: userResult._id, status: { $ne: status.DELETE }, orderPlace: true, isSold: false });
    ...
    async soldOrder(req, res, next) {
        try {
            ...
            let dataResults = await soldOrderList({ userId: userResult._id, status: { $ne: status.DELETE }, isSold: true });
    ...
    */
    ...
    async buyOrder(req, res, next) {
        try {
            ...
            let dataResults = await soldOrderList({ buyerId: userResult._id, userId: userResult._id, status: { $ne: status.DELETE } });
            if (dataResults.length === 0) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(dataResults, responseMessage.DATA_FOUND));
    ...
    async auctionNftList(req, res, next) {
        try {
            let dataResults = await auctionNftList({ status: { $ne: status.DELETE } });
            return res.json(new response(dataResults, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
    ...
    async allListOrder(req, res, next) {
        try {
            let dataResults = await orderList({ status: { $ne: status.DELETE }, isCancel: { $ne: true }, isSold: { $ne: true }, orderPlace: true, isExport: false });
            return res.json(new response(dataResults, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
    ...
    async sendOrderToUser(req, res, next) {
        ...
        try {
            ...
            if (!orderRes) {
                ...
                var userEarn = {};
                var userEarningResult = await findEarning({ userId: orderRes.userId, status: status.ACTIVE });
                if (!userEarningResult) {
                    ...
                    await updateEarning({ _id: userEarningResult._id }, { massBalance: Number(nftRes.startingBid) })
                }
                if (userRes.userType == "Creator") {
                    ...
                    let orderUpdate = await updateOrder({ _id: orderRes._id }, { buyerId: userRes._id, isSold: true, isBuy: true, orderPlace: false, bidId: [] });
                    await updateAuctionNft({ _id: nftRes._id }, { userId: userRes._id, orderPlace: false })
                    let updateQuery1 = {};
                    ...
                    await updateAuctionNft({ _id: nftRes._id }, { buyerId: userRes._id, auctionStatus: "STOPPED", isSold: false, isBuy: false, });
                    ...
                    let activity = await nftHistoryModel(obj).save();
                    await createTransaction({
                        userId: validatedBody.userId,
                        nftId: orderRes.nftId,
                        nftUserId: validatedBody.sellerId,
    ...
    async cancelOrder(req, res, next) {
        ...
        try {
            ...
            var orderResult = await findOrder({ _id: _id, isCancel: false, status: { $ne: status.DELETE } });
            if (!orderResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            var result = await updateOrder({ _id: orderResult._id }, { isCancel: true, orderPlace: false });
            await updateAuctionNft({ _id: result.nftId }, { orderPlace: false }, { $inc: { treandingNftCount: 0 } });
