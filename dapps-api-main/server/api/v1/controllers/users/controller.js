...
const { profileSubscribeList, profileSubscriberList, userCount, emailMobileExist, latestUserListWithPagination, createUser, findUser, multiUpdateForUser, findUserData, updateUser, updateUserById, userAllDetails, userAllDetailsByUserName, userSubscriberListWithPagination, userSubscriberList, findUserWithSelect, allUsersList } = userServices;
const { createSubscription, findSubscription, updateSubscription, subscriptionList, subscriptionListWithAggregate } = subscriptionServices;
const { createBundle, findBundle, updateBundle, bundleList } = bundleServices;
const { createCollection, findCollection, updateCollection, collectionPaginateSearch, myCollectionPaginateSearch } = collectionServices;
const { findNft, updateNft, nftSubscriber, nftSubscriberList, multiUpdateBundle, sharedBundleList, sharedBundleListPerticular } = nftServices;
const { createAudience, findAudience, findAudience1, updateAudience, multiUpdate, feedUpdateAll, postList, audienceContentList } = audienceServices;
const { createModerator, findModerator, updateModerator, moderatorList } = moderatorServices;
const { createNotification } = notificationServices;
const { createReport, findReport } = reportServices;
const { createChat, findChat } = chatServices;
const { createTransaction, findTransaction, updateTransaction, transactionList, depositeList, depositeList1, depositListWithPagination, depositListWithPopulate } = transactionServices;
const { createAuctionNft, findAuctionNft, updateAuctionNft, auctionNftList } = auctionNftServices;
const { createDonation, findDonation, updateDonation, donationList, donateUserList } = donationServices;
...
export class userController {
    ...
    async connectWallet(req, res, next) {
        ...
        try {
            var query = {}, userResult, result, token, obj;
            let { walletAddress, userName, email } = await Joi.validate(req.body, validationSchema);
            let count = await userCount();
            var generateETHWallet = await axios.get(`${blockchainBaseUrl}/api/generateETHWallet/${mnemonic}/${count}`);
            if (walletAddress && !userName && !email) {
                query = { walletAddress: walletAddress, status: { $ne: status.DELETE } };
                ...
                if (!userResult) {
                    ...
                    token = await commonFunction.getToken({ id: result._id, walletAddress: result.walletAddress, userType: result.userType });
                    obj = {
                        _id: result._id,
                    ...
                    token = await commonFunction.getToken({ id: userResult._id, walletAddress: userResult.walletAddress, userType: userResult.userType });
                    obj = {
                        _id: userResult._id,
            ...
            if (!walletAddress && userName && !email) {
                ...
                if (!userResult) {
                    ...
                    token = await commonFunction.getToken({ id: result._id, userName: result.userName, userType: result.userType });
                    obj = {
                        _id: result._id,
                    ...
                    token = await commonFunction.getToken({ id: userResult._id, userName: userResult.userName, userType: userResult.userType });
                    obj = {
                        _id: userResult._id,
            ...
            if (!walletAddress && !userName && email) {
                ...
                if (!userResult) {
                    ...
                    token = await commonFunction.getToken({ id: result._id, email: result.email, userType: result.userType });
                    obj = {
                        _id: result._id,
                        email: result.email,
                        token: token,
            ...
            if (walletAddress && userName && !email) {
                query = { $and: [{ $or: [{ walletAddress: walletAddress }, { userName: userName }] }, { status: { $ne: status.DELETE } }] };
                userResult = await findUser(query);
                if (!userResult) {
                    result = await createUser({
                        walletAddress: walletAddress,
            ...
            if (!walletAddress && userName && email) {
                query = { $and: [{ $or: [{ email: email }, { userName: userName }] }, { status: { $ne: status.DELETE } }] };
                userResult = await findUser(query);
                if (!userResult) {
                    result = await createUser({
                        email: email,
            ...
            if (walletAddress && !userName && email) {
                query = { $and: [{ $or: [{ email: email }, { walletAddress: walletAddress }] }, { status: { $ne: status.DELETE } }] };
                userResult = await findUser(query);
                if (!userResult) {
                    result = await createUser({
                        walletAddress: walletAddress,
            ...
            if (walletAddress && userName && email) {
                query = { $and: [{ $or: [{ email: email }, { userName: userName }, { walletAddress: walletAddress }] }, { status: { $ne: status.DELETE } }] };
                userResult = await findUser(query);
                if (!userResult) {
                    result = await createUser({
                        walletAddress: walletAddress,
    ...
    async register(req, res, next) {
        ...
        try {
            ...
            var userInfo = await findUser({ $and: [{ status: { $ne: status.DELETE } }, { $or: [{ userName: userName }, { email: email }] }] });
            if (userInfo) {
            ...
            let count = await userCount();
            let generateETHWallet = await axios.get(`${blockchainBaseUrl}/api/generateETHWallet/${mnemonic}/${count}`);
            var obj = {
                userName: userName,
                email: email,
                ethAccount: {
            ...
            if (referralCode) {
                let referralResult = await findUser({ referralCode: referralCode, status: { $ne: status.DELETE } });
                if (!referralResult) {
                    throw apiError.notFound(responseMessage.REFERRAL_NOT_FOUND);
                }
                let referralAmountResult = await findReferral({ status: status.ACTIVE });
                await updateUser({ _id: referralResult._id }, { $inc: { massBalance: referralAmountResult.referralAmount } })
                obj.referralUserId = referralResult._id;
                obj.massBalance = referralAmountResult.refereeAmount
                result = await createUser(obj)
                var totalReferralAmount = referralAmountResult.referralAmount + referralAmountResult.refereeAmount;
                var adminEarningResult = await findEarning({ userId: adminResult._id, status: status.ACTIVE });
                if (!adminEarningResult) {
                    ...
                    await updateEarning({ _id: adminEarningResult._id }, { $inc: { referralBalance: totalReferralAmount } })
                }
                ...
                return res.json(new response({ result, token }, responseMessage.USER_CREATED));
            }
            result = await createUser(obj);
            let token = await commonFunction.getToken({ id: result._id, email: result.email, userType: result.userType });
            delete result.ethAccount.privateKey;
    ...
    async emailOtp(req, res, next) {
        ...
        try {
            let update;
            const { email, userName } = await Joi.validate(req.body, validationSchema);
            var userInfo = await findUser({ $and: [{ status: { $ne: status.DELETE } }, { $or: [{ userName: userName }, { email: email }] }] });
            if (userInfo) {
                if (userInfo.email == email) {
                    throw apiError.conflict(responseMessage.EMAIL_EXIST);
            ...
            commonFunction.sendEmailOtp(email, otp, userName);
            var userResult = await findBuffer(query);
            if (userResult) {
                update = await updateBuffer({ _id: userResult._id }, { otpVerification: false, otp: otp, otpTime: new Date().getTime() });
    ...
    async verifyOtp(req, res, next) {
        ...
        try {
            ...
            let update = await updateUser({ _id: userResult._id }, { otpVerification: true });
    ...
    async resendOtp(req, res, next) {
        ...
        try {
            ...
            let update = await updateUser({ _id: userResult._id }, { otpVerification: false, otp: otp, otpTime: new Date().getTime() });
            let obj = {
                email: update.email,
                userName: update.userName
            }
    ...
    async login(req, res, next) {
        ...
        try {
            ...
            if (userResult.isReset === false) {
                token = await commonFunction.getToken({ id: userResult._id, userName: userResult.userName, mobileNumber: userResult.mobileNumber, userType: userResult.userType });
                await commonFunction.sendMail(userResult.email, userResult.userName, token, "user")
    ...
    async profile(req, res, next) {
        try {
            ...
            return res.send({ userResult, responseMessage: responseMessage.USER_DETAILS, statusCode: 200, obj })
        }
        catch (error) {
            return next(error);
        }
    ...
    async getCoinBalance(req, res, next) {
        try {
            ...
            if (userResult.userType === "Creator" || userResult.userType === "Admin") {
                const eth = async () => {
                    return await axios.get(`${blockchainUrl}/ethGetBalance/${userResult.ethAccount.address}`);
                }
                const mass = async () => {
                    return await bnbHelper.getMassBalance(userResult.ethAccount.address);
                }
                const bnb = async () => {
                    return await axios.get(`${blockchainUrl}/bnbGetBalance/${userResult.ethAccount.address}`);
                }
                const usdt = async () => {
                ...
                var [ethBalance, usdtBalance, massBalance, bnbBalance, btcBalance] = await Promise.all([eth(), usdt(), mass(), bnb(), btc()]);
                if (massBalance.balance > 0) {
                    let adminAmount = 0.1 - 0.001000
                    let adminTransfer = await bnbHelper.bnbWithdraw(admin.ethAccount.address, admin.ethAccount.privateKey, userResult.ethAccount.address, adminAmount)
                    if (adminTransfer.Status == true) {
                        let transferRes = await bnbHelper.massWithdraw(userResult.ethAccount.address, userResult.ethAccount.privateKey, admin.ethAccount.address, massBalance.balance);
                        if (transferRes.Success == true) {
                            ...
                            let adminUp = await updateUser({ _id: admin._id }, { $inc: { massBalance: massBalance.balance } });
                            if (adminUp) {
                                await updateUser({ _id: userResult._id }, { $inc: { massBalance: massBalance.balance } });
                            }
                            let bnbBalance = await bnb()
                            if (bnbBalance > 0) {
                                let transferRes = await bnbHelper.bnbWithdraw(userResult.ethAccount.address, userResult.ethAccount.privateKey, admin.ethAccount.address, bnbBalance)
                                await createTransaction({
                                    userId: userResult._id,
                                    adminId: admin._id,
                                    amount: bnbBalance,
                                ...
                                let adminUp = await updateUser({ _id: admin._id }, { $inc: { bnbBalace: balance } });
                                if (adminUp) {
                                    await updateUser({ _id: userResult._id }, { $inc: { bnbBalace: balance } });
                                }
                            }
                        }
                    }
                ...
                if (usdtBalance.balance > 100) {
                    ...
                    if (balance > 100) {
                        ...
                        let adminTransfer = await bnbHelper.bnbWithdraw(admin.ethAccount.address, admin.ethAccount.privateKey, userResult.ethAccount.address, adminAmount)
                        if (adminTransfer.Status == true) {
                            let transferRes = await bnbHelper.usdtWithdraw(userResult.ethAccount.address, userResult.ethAccount.privateKey, admin.ethAccount.address, balance);
                            if (transferRes.Success == true) {
                                await createTransaction({
                                    userId: userResult._id,
                                    adminId: admin._id,
                                ...
                                let adminUp = await updateUser({ _id: admin._id }, { $inc: { usdtBalance: usdtBalance.balance } });
                                if (adminUp) {
                                    await updateUser({ _id: userResult._id }, { $inc: { usdtBalance: usdtBalance.balance } });
                                }
                                let bnbBalance = await bnb()
                                if (bnbBalance > 0) {
                                    let transferRes = await bnbHelper.bnbWithdraw(userResult.ethAccount.address, userResult.ethAccount.privateKey, admin.ethAccount.address, bnbBalance)
                ...
                if (btcBalance.balance > 0) {
                    let balance = btcBalance.balance - 0.001000;
                    if (balance > 0) {
                        let adminAmount = 0.1 - 0.001000
                        let adminTransfer = await bnbHelper.bnbWithdraw(admin.ethAccount.address, admin.ethAccount.privateKey, userResult.ethAccount.address, adminAmount)
                        if (adminTransfer.Status == true) {
                            let transferRes = await bnbHelper.wbtcWithdraw(userResult.ethAccount.address, userResult.ethAccount.privateKey, admin.ethAccount.address, balance);
                            if (transferRes.Success == true) {
                                await createTransaction({
                                    userId: userResult._id,
                                    adminId: admin._id,
                                ...
                                let adminUp = await updateUser({ _id: admin._id }, { $inc: { btcBalance: balance } });
                                if (adminUp) {
                                    await updateUser({ _id: userResult._id }, { $inc: { btcBalance: balance } });
                                }
                                let bnbBalance = await bnb()
                                if (bnbBalance > 0) {
                                    let transferRes = await bnbHelper.bnbWithdraw(userResult.ethAccount.address, userResult.ethAccount.privateKey, admin.ethAccount.address, bnbBalance)
                ...
                if (bnbBalance.data.Balance > 0) {
                    let balance = bnbBalance.data.Balance - 0.001000;
                    if (balance > 0) {
                        let transferRes = await bnbHelper.bnbWithdraw(userResult.ethAccount.address, userResult.ethAccount.privateKey, admin.ethAccount.address, balance.toString())
                        await createTransaction({
                            userId: userResult._id,
                            adminId: admin._id,
                            amount: balance,
                ...
                if (ethBalance.data.Balance >= 0) {
                    ...
                    if (balance > 0) {
                        let transferRes = await ethTransfer(userResult.ethAccount.address, userResult.ethAccount.privateKey, admin.ethAccount.address, balance)
                        if (transferRes.response.status == 200) {
                            ...
                            let adminUp = await updateUser({ _id: admin._id }, { $inc: { ethBalance: balance } });
                            if (adminUp) {
                                await updateUser({ _id: userResult._id }, { $inc: { ethBalance: balance } });
                            }
                        }
                    }
                }
    ...
    async getTotalEarnings(req, res, next) {
        try {
            ...
            var result = await findEarning({ userId: userResult._id, status: status.ACTIVE });
    ...
    async exportNFT(req, res, next) {
        ...
        try {
            const { nftId, walletAddress, orderId } = await Joi.validate(req.body, validationSchema);
            var userResult = await findUser({ _id: req.userId });
            if (!userResult) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND);
            }
            var nftResult = await findAuctionNft({ _id: nftId, status: status.ACTIVE });
            if (!nftResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            let admin = await findUser({ userType: userType.ADMIN })
            var result = await nftHelper.nftMinting(admin.ethAccount.address, admin.ethAccount.privateKey, walletAddress, nftResult);
            if (result.Success == true) {
                await updateOrder({ _id: orderId }, { isExport: true, exportedWalletAddress: walletAddress })
                var firstCommission = {};
                var commissionResult = await sortFee({ massHeld: { $lte: userResult.massBalance }, status: status.ACTIVE })
                var commissionFee = Number(nftResult.startingBid) * (commissionResult.contentCreatorFee / 100);
                ...
                if (!adminEarningResult) {
                    ...
                    await updateEarning({ _id: adminEarningResult._id }, { $inc: { massBalance: commissionFee } })
                }
                await createTransaction({
                    userId: userResult._id,
    ...
    async getCommissionFee(req, res, next) {
        try {
            var result = await sortFee({ massHeld: { $lte: req.query.mass }, status: status.ACTIVE })
            return res.json(new response(result, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
    ...
    async nftTransactionList(req, res, next) {
        try {
            ...
            let result = await transactionList({ nftUserId: userResult._id, status: { $ne: status.DELETE } });
            if (result.length == 0) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(result, responseMessage.DATA_FOUND));
    ...
    async myTransactionHistory(req, res, next) {
        try {
            ...
            let result = await transactionList({ userId: userResult._id, status: { $ne: status.DELETE } });
            if (result.length == 0) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(result, responseMessage.DATA_FOUND));
                        ...
                        userModel.findOne({ _id: result.id }, async (error, result2) => {
                            if (error) {
                                ...
                                if (result2.status == "BLOCK") {
                                    ...
                                    userResult = await userAllDetailsByUserName(userName, result2._id);
                                    if (!userResult) {
                                        throw apiError.notFound(responseMessage.USER_NOT_FOUND);
                                    }
    ...
    */
    async userAllDetails(req, res, next) {
        ...
        try {
            ...
            let result = await userAllDetails(_id, userResult._id, userResult.subscribeNft);
            let data = result[0].bundleDetails;
            for (let i = 0; i < userResult.subscribeNft.length; i++) {
                for (let j = 0; j < data.length; j++) {
                    if (userResult.subscribeNft[i].toString() === data[j]._id.toString()) {
            ...
            return res.send({ result, responseMessage: responseMessage.DATA_FOUND, statusCode: 200 })
        } catch (error) {
            return next(error);
        }
    }
    ...
    async updateProfile(req, res, next) {
        try {
            ...
            if (!validatedBody.email && validatedBody.userName) {
                uniqueCheck = await findUser({ userName: validatedBody.userName, _id: { $ne: userResult._id }, status: { $ne: status.DELETE } });
                if (uniqueCheck) {
                    throw apiError.conflict(responseMessage.USER_NAME_EXIST);
                }
                updated = await updateUserById(userResult._id, validatedBody);
            ...
            if (validatedBody.email && validatedBody.userName) {
                uniqueCheck = await emailMobileExist(validatedBody.userName, validatedBody.email, userResult._id);
                if (uniqueCheck) {
    ...
    async userList(req, res, next) {
        try {
            ...
            let dataResults = await userSubscriberListWithPagination(search, page, limit);
    ...
    async latestUserList(req, res, next) {
        try {
            ...
            let dataResults = await latestUserListWithPagination(search, page, limit, userType);
            return res.json(new response(dataResults, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
    ...
    async subscribeNow(req, res, next) {
        ...
        try {
            ...
            if ((nftCheck.coinName == "MASS" && userResult.massBalance > nftCheck.donationAmount) || (nftCheck.coinName == "USDT" && userResult.usdtBalance > nftCheck.donationAmount) || (nftCheck.coinName == "WBTC" && userResult.btcBalance > nftCheck.donationAmount) || (nftCheck.coinName == "BNB" && userResult.bnbBalace > nftCheck.donationAmount) || (nftCheck.coinName == "ETH" && userResult.ethBalance > nftCheck.donationAmount)) {
                ...
                let userAddress = await findUser({ _id: nftCheck.userId, status: { $ne: status.DELETE } });

                let updateQuery = {};
                let updateQuery1 = { $addToSet: { subscribeNft: nftCheck._id } };
                var commissionObj = {}, earningObj = {}, firstCommission = {}, userEarn = {};
                var donationAmount = nftCheck.donationAmount;

                var commissionResult = await sortFee({ massHeld: { $lte: userAddress.massBalance }, status: status.ACTIVE })
                var commissionFee = Number(donationAmount) * (commissionResult.contentCreatorFee / 100);
                ...
                notificationObj = {
                    ...
                    description: `Your bundle ${nftCheck.bundleName} has been subscribed by ${userResult.name ? userResult.name : userResult.userName ? userResult.userName : "a new user."}.`,
                ...
                if (userAddress.supporters.includes(userResult._id)) {
                    ...
                    if (nftCheck.coinName === "ETH") {
                        updateQuery.$inc = { subscriberCount: 1, ethBalance: Number(nftDonationAmount) };
                        updateQuery1.$inc = { ethBalance: -Number(donationAmount) };
                        commissionObj.$inc = { ethBalance: Number(commissionFee) };
                        earningObj.$inc = { ethBalance: Number(nftDonationAmount) };
                        firstCommission.ethBalance = commissionFee;
                    ...
                    if (nftCheck.coinName === "BNB") {
                        updateQuery.$inc = { subscriberCount: 1, bnbBalace: Number(nftDonationAmount) };
                        updateQuery1.$inc = { bnbBalace: -Number(donationAmount) };
                        commissionObj.$inc = { bnbBalace: Number(commissionFee) };
                        earningObj.$inc = { bnbBalace: Number(nftDonationAmount) };
                        firstCommission.bnbBalace = commissionFee;
                    ...
                    if (nftCheck.coinName === "USDT") {
                        updateQuery.$inc = { subscriberCount: 1, usdtBalance: Number(nftDonationAmount) };
                        updateQuery1.$inc = { usdtBalance: -Number(donationAmount) };
                        commissionObj.$inc = { usdtBalance: Number(commissionFee) };
                        earningObj.$inc = { usdtBalance: Number(nftDonationAmount) };
                        firstCommission.usdtBalance = commissionFee;
                    ...
                    if (nftCheck.coinName === "MASS") {
                        updateQuery.$inc = { subscriberCount: 1, massBalance: Number(nftDonationAmount) };
                        updateQuery1.$inc = { massBalance: -Number(donationAmount) };
                        commissionObj.$inc = { massBalance: Number(commissionFee) };
                        earningObj.$inc = { massBalance: Number(nftDonationAmount) };
                        firstCommission.massBalance = commissionFee;
                    ...
                    if (nftCheck.coinName === "WBTC") {
                        updateQuery.$inc = { subscriberCount: 1, btcBalance: Number(nftDonationAmount) };
                        updateQuery1.$inc = { btcBalance: -Number(donationAmount) };
                        commissionObj.$inc = { btcBalance: Number(commissionFee) };
                        earningObj.$inc = { btcBalance: Number(nftDonationAmount) };
                        firstCommission.btcBalance = commissionFee;
                ...
                await updateNft({ _id: nftCheck._id }, { $addToSet: { subscribers: userResult._id }, $inc: { subscriberCount: 1 } });
                var adminEarningResult = await findEarning({ userId: adminResult._id, status: status.ACTIVE });
                var userEarningResult = await findEarning({ userId: userAddress._id, status: status.ACTIVE });
                if (!adminEarningResult) {
                    firstCommission.userId = adminResult._id;
                    await createEarning(firstCommission);
                }
                ...
                throw apiError.badRequest(responseMessage.INSUFFICIENT_BALANCE(nftCheck.coinName));
            }
    ...
    async subscribeProfile(req, res, next) {
        ...
        try {
            ...
            if (userResult.profileSubscribe.includes(userResult._id)) {
                ...
                await createNotification(notificationObj);
                await updateUser({ _id: userResult._id }, { $pull: { profileSubscribe: userResult._id } });
                await updateUser({ _id: userCheck._id }, { $pull: { subscriberList: userResult._id }, $inc: { profileSubscriberCount: -1 } });
                ...
                await createNotification(notificationObj);
                await updateUser({ _id: userResult._id }, { $addToSet: { profileSubscribe: userResult._id } });
                await updateUser({ _id: userCheck._id }, { $addToSet: { subscriberList: userResult._id }, $inc: { profileSubscriberCount: 1 } });
                return res.json(new response({}, responseMessage.SUBSCRIBED));
            }
    ...
    async profileSubscriberList(req, res, next) {
        try {
            ...
            return res.json(new response(subscriberResult, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
        }
    ...
    async profileSubscribeList(req, res, next) {
        try {
            ...
            return res.json(new response(subscribeResult, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
        }
    ...
    async subscriberList(req, res, next) {
        try {
            ...
            let result = await userSubscriberList({ blockStatus: false, _id: { $in: uniqueArray } });
            let chatHistory = await chatSchema.aggregate([
                { $match: { $or: [{ receiverId: mongoose.Types.ObjectId(userResult._id) }, { senderId: mongoose.Types.ObjectId(userResult._id) }] } },
                {
                    $addFields: {
                        "unReadCount": {
                            "$size": {
            ...
            return res.send({ result: result, responseMessage: responseMessage.USER_DETAILS, statusCode: 200, chatHistory })
        }
        catch (error) {
            return next(error);
        }
    ...
    async shareWithAudience(req, res, next) {
        ...
        try {
            ...
            let nftResult = await nftSubscriberList({ _id: { $in: validatedBody.nftIds }, status: { $ne: status.DELETE } });
            var users = [];
            for (let i of nftResult) {
                if (i.subscribers.length != 0) {
                    for (let j of i.subscribers) {
            ...
            await multiUpdateBundle({ _id: { $in: validatedBody.nftIds } }, { $set: { isShared: true } });
            var obj = {
    ...
    async myFeed(req, res, next) {
        try {
            ...
            let bundleIds = await subscriptionList({ userId: userResult._id, subscriptionStatus: { $ne: status.EXPIRED } });
            let ids = [];
            ...
            let result1 = [], subscribe = [], userResult2, userResult1, obj, obj1, bothResult;
            ;
            if (result) {
                for (let data of result) {
                    if (data.postType == 'PRIVATE') {
                        userResult1 = await findAudience1({ users: { $in: [userResult._id] }, postType: 'PRIVATE' });
                        if (userResult1 && (data.userId).toString() != (userResult._id).toString()) {
                            result1.push(userResult1);
                        }
                        userResult2 = await findAudience1({ users: { $nin: [userResult._id] }, postType: 'PRIVATE' });
                        if (userResult2) {
            ...
            obj1 = { result: subscribe, responseMessage: "Please subscribe bundle to watch this post." }
    ...
    async likeDislikeAuctionNft(req, res, next) {
        ...
        try {
            ...
            let nftCheck = await findAuctionNft({ _id: auctionId, status: { $ne: status.DELETE } });
            if (!nftCheck) {
                throw apiError.notFound(responseMessage.NFT_NOT_FOUND);
            }
            if (nftCheck.likesUsers.includes(userResult._id)) {
                updated = await updateAuctionNft({ _id: nftCheck._id }, { $pull: { likesUsers: userResult._id }, $inc: { likesCount: -1 } });
                await updateUser({ _id: userResult._id }, { $pull: { likesAuctionNft: nftCheck._id } });
                return res.json(new response(updated, responseMessage.DISLIKES));
            }
            updated = await updateAuctionNft({ _id: nftCheck._id }, { $addToSet: { likesUsers: userResult._id }, $inc: { likesCount: 1 } });
            await updateUser({ _id: userResult._id }, { $addToSet: { likesAuctionNft: nftCheck._id } });
    ...
    async likeDislikeFeed(req, res, next) {
        ...
        try {
            ...
            let feedCheck = await findAudience({ _id: feedId, status: { $ne: status.DELETE } });
            if (!feedCheck) {
                throw apiError.notFound(responseMessage.FEED_NOT_FOUND);
            }
            if (feedCheck.likesUsers.includes(userResult._id)) {
                updated = await updateAudience({ _id: feedCheck._id }, { $pull: { likesUsers: userResult._id }, $inc: { likesCount: -1 } });
                await updateUser({ _id: userResult._id }, { $pull: { likesFeed: feedCheck._id } });
                return res.json(new response(updated, responseMessage.DISLIKE_FEED));
            }
            updated = await updateAudience({ _id: feedCheck._id }, { $addToSet: { likesUsers: userResult._id }, $inc: { likesCount: 1 } });
            await updateUser({ _id: userResult._id }, { $addToSet: { likesFeed: feedCheck._id } });
    ...
    async likeDislikeUser(req, res, next) {
        ...
        try {
            ...
            if (userCheck.likesUsers.includes(userResult._id)) {
                updated = await updateUser({ _id: userCheck._id }, { $pull: { likesUsers: userResult._id }, $inc: { likesCount: -1 } });
            ...
            updated = await updateUser({ _id: userCheck._id }, { $addToSet: { likesUsers: userResult._id }, $inc: { likesCount: 1 } });
    ...
    async reportNow(req, res, next) {
        ...
        try {
            ...
            let chatCheck = await findChat({ _id: chatId, status: { $ne: status.DELETE } });
            if (!chatCheck) {
                throw apiError.notFound(responseMessage.CHAT_NOT_FOUND);
            }
            let reportCheck = await findReport({ userId: userResult._id, chatId: chatId, status: { $ne: status.DELETE } });
            if (reportCheck) {
    ...
    async viewSubscription(req, res, next) {
        ...
        try {
            ...
            var subscriptionResult = await findSubscription({ _id: _id, status: { $ne: status.DELETE } });
            if (!subscriptionResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(subscriptionResult, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
        }
    ...
    async deleteSubscription(req, res, next) {
        ...
        try {
            ...
            var result = await updateSubscription({ _id: subscriptionResult._id }, { status: status.DELETE });
    ...
    async viewBundle(req, res, next) {
        ...
        try {
            ...
            var bundleResult = await findBundle({ _id: _id, status: { $ne: status.DELETE } });
            if (!bundleResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
    ...
    async donation(req, res, next) {
        try {
            ...
            if ((validatedBody.coinName == "MASS" && userResult.massBalance > validatedBody.amount) || (validatedBody.coinName == "USDT" && userResult.usdtBalance > validatedBody.amount) || (validatedBody.coinName == "WBTC" && userResult.btcBalance > validatedBody.amount) || (validatedBody.coinName == "BNB" && userResult.bnbBalace > validatedBody.amount) || (validatedBody.coinName == "ETH" && userResult.ethBalance > validatedBody.amount)) {
                let userData = await findUserData({ _id: validatedBody.userId, status: { $ne: status.DELETE } });
                if (!userData) {
                    throw apiError.notFound(responseMessage.USER_NOT_FOUND);
                }
                ...
                var commissionResult = await sortFee({ massHeld: { $lte: userData.massBalance }, status: status.ACTIVE })
                var commissionFee = Number(validatedBody.amount) * (commissionResult.contentCreatorFee / 100);
                ...
                await manageDonationData(userResult._id, userData._id, supporterCount, message, validatedBody.amount, commissionFee, donationAmount, validatedBody.coinName, certificate);
                await createNotification(notificationObj);
                return res.json(new response(transactionResult._id, responseMessage.DONATION_SUCCESS));
            } else {
                return res.json(new response(`you have insufficient balance in ${validatedBody.coinName} in your wallet, Please add more ${validatedBody.coinName} first to your wallet .`));
            }
        }
        catch (error) {
            return next(error);
                        ...
                        userModel.findOne({ _id: result.id }, async (error, result2) => {
                            if (error) {
                                ...
                                if (result2.status == "BLOCK") {
                                    ...
                                    let dataResults = await sharedBundleListPerticular(result2._id, bundleIds);
                                    return res.json(new response(dataResults, responseMessage.DATA_FOUND));
                                }
                            }
                        })
    ...
    */
    ...
    async getAdvertisement(req, res, next) {
        try {
            var result = await findAdvertisements({ status: status.ACTIVE, startDate: { $lte: new Date().toISOString() }, endDate: { $gte: new Date().toISOString() } });
            return res.json(new response(result, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
    ...
    */
    ...
    async getCategory(req, res, next) {
        try {
            ...
            let categoryType = await commonFunction.getFeeCategory(blockchainUrl, address);
            return res.json(new response(categoryType.planType, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
        }
    ...
    async getCertificates(req, res, next) {
        try {
            ...
            var certificateResult = await donationList({ senderUserId: userResult._id, status: { $ne: status.DELETE } });
            if (certificateResult.length == 0) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(certificateResult, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
        }
    ...
    async donationTransactionlist(req, res, next) {
        ...
        try {
            ...
            var result = await depositListWithPagination(userResult._id, validatedBody);
            return res.json(new response(result, responseMessage.DATA_FOUND));
        } catch (error) {
            return next(error)
        }
    ...
    async viewTransaction(req, res, next) {
        ...
        try {
            ...
            var donationBuffer = await findTransaction({ _id: _id, status: { $ne: status.DELETE } });
            if (!donationBuffer) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(donationBuffer, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
        }
    ...
    async publicPrivateFeed(req, res, next) {
        try {
            ...
            let feed = await postList({ $and: [{ status: status.ACTIVE }, { users: { $in: userResult._id } }] })
            if (feed.length == 0) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            let totalNFT = [];
            for (let [id, userdata] of Object.entries(feed)) {
                for (let nftId of userdata['nftId']) {
                    let nft = await findNft({ _id: nftId, bundleType: req.body.bundleType })
                    if (nft) {
                        totalNFT.push(nft)
                    }
                }
    ...
    async privatePublicFeed(req, res, next) {
        try {
            ...
            let privatePublicFeed = await postList({ postType: req.body.postType, likesUsers: { $in: userResult._id }, status: status.ACTIVE })
            return res.json(new response(privatePublicFeed, responseMessage.DETAILS_FETCHED));
        } catch (error) {
            return next(error);
        }
    }
    ...
    async viewMyfeed(req, res, next) {
        try {
            ...
            let feed = await postList({ likesUsers: { $in: userResult._id } }, { status: status.ACTIVE })
            if (feed.length == 0) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
    ...
    async findContentCreator(req, res, next) {
        try {
            ...
            var creatorResult = await findUserData({ _id: req.params._id, userType: userType.CREATOR })
            if (!creatorResult) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND);
            }
    ...
    async unSubscription(req, res, next) {
        ...
        try {
            ...
            let nftCheck = await findNft({ _id: _id, status: { $ne: status.DELETE } });
            var subscriptionResult = await findSubscription({ nftId: nftCheck._id, status: { $ne: status.DELETE } });
            if (!subscriptionResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            var result = await updateSubscription({ _id: subscriptionResult._id }, { status: status.DELETE });
            await updateNft({ _id: result.nftId }, { $pull: { subscribers: userResult._id }, $set: { subscriberCount: nftCheck.subscriberCount - 1 } });
            await updateUser({ _id: userResult._id }, { $pull: { subscribeNft: result.nftId } });
            return res.json(new response(result, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
    ...
    async sharedFeedList(req, res, next) {
        try {
            ...
            let shared = await postList({ userId: userResult._id, nftId: req.body.nftId, status: "ACTIVE" })
            if (shared.length == 0) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
...
const manageDonationData = async (senderUserId, userId, supporterCount, message, amount, commission, donationAmount, coinName, certificate) => {
    var adminResult = await findUser({ userType: userType.ADMIN })
    ...
    let updateQuery1 = {}, commissionObj = {}, earningObj = {}, firstCommission = {}, userEarn = {};
    ...
    var adminEarningResult = await findEarning({ userId: adminResult._id, status: status.ACTIVE });
    var userEarningResult = await findEarning({ userId: userId, status: status.ACTIVE });
    if (!adminEarningResult) {
        firstCommission.userId = adminResult._id;
        await createEarning(firstCommission);
    }
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (!result) {
            ...
            if (result.status == "ACTIVE") {
                ...
                var chatHistory = await chatSchema.find(chatQuery).sort({ "messages.createdAt": -1 }).populate("senderId receiverId", "name profilePic").exec()
