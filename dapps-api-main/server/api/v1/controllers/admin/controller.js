...
const { createUser, findUser, updateUser, userAllDetailsWithBundleCount, userCount, adminModeratorList, adminPaginationList, userList, listSubAdmin, usersFundAggregate } = userServices;
...
const { createOrder, findOrder, findOrderWithPopulate, updateOrder, orderList, orderCount } = orderServices;
const { createBid, findBid, updateBid, bidList, bidCount } = bidServices;
const { createModerator, findModerator, updateModerator, moderatorList } = moderatorServices;
const { createReport, findReport, updateReport, reportList } = reportServices;
...
const { createTransaction, findTransaction, updateTransaction, transactionList, depositeList, allTransactions } = transactionServices;
const { createLogo, findLogo, updateLogoById, logoList } = logoServices;
const { createDonation, findDonation, updateDonation, donationList } = donationServices;
const { createPress, findPress, updatePress, paginatePressList, pressList } = pressServices;
const { createPartner, findPartner, updatePartner, paginatePartnerList, partnerList } = partnerServices;
const { createAdvertisement, findAdvertisement, findAllAdvertisement, updateAdvertisement, updateAdvertisementById, paginateSearchAdvertisement } = advertisementServices;
const { createSocial, findSocial, findAllSocial, updateSocial, updateSocialById, paginateSearchSocial } = socialServices;
const { findEarning } = earningServices;
const { findReferral, updateReferral } = referralServices;
const { createBanner, findBanner, updateBanner, paginateSearchBanner } = bannerServices;
...
export class adminController {
    ...
    async addAdmin(req, res, next) {
        ...
        try {
            ...
            const { userName, password, email, permissions, ip } = validatedBody;
    ...
    async register(req, res, next) {
        ...
        try {
            ...
            if (referralCode) {
                ...
                transferRes = await axios.post(`${blockchainUrl}/withdraw`, transactionObj);
   ...
   */
    ...
    async setPermissions(req, res, next) {
        ...
        try {
            ...
            for (let id of validatedBody._id) {
                await updateUser({ _id: id }, { permissions: validatedBody.permissions });
            }
    ...
    async login(req, res, next) {
        ...
        try {
            ...
            let query = { $and: [{ userType: { $in: [userType.ADMIN, userType.SUB_ADMIN] } }, { $or: [{ email: email }, { userName: email }] }] }
    ...
    async forgotPassword(req, res, next) {
        ...
        try {
            ...
            var token = await commonFunction.getToken({ id: userResult._id, userName: userResult.userName, mobileNumber: userResult.mobileNumber, userType: userResult.userType });
            await commonFunction.sendMail(userResult.email, userResult.userName, token, type)
            var updateResult = await updateUser({ _id: userResult._id }, { isReset: false })
    ...
    async userList(req, res, next) {
        const validationSchema = {
            ...
            planType: Joi.string().valid(planType.BASIC, planType.SILVER, planType.GOLD, planType.DIAMOND, planType.MASS_PLUS).optional(),
            fromDate: Joi.string().optional(),
            toDate: Joi.string().optional(),
            page: Joi.number().optional(),
            limit: Joi.number().optional(),
    ...
    async viewUser(req, res, next) {
        ...
        try {
            ...
            var subscriptionResult = await findUser({ _id: _id, status: { $ne: status.DELETE } });
    ...
    async addSubscription(req, res, next) {
        ...
        try {
            const validatedBody = await Joi.validate(req.body, validationSchema);
            const { title, description, validTillDate, masPrice, fees } = validatedBody;

            var subscriptionResult = await findSubscription({ title: title, status: { $ne: status.DELETE } });
            if (subscriptionResult) {
    ...
    async editSubscription(req, res, next) {
        ...
        try {
            ...
            var subscriptionResult = await findSubscription({ _id: validatedBody._id, status: { $ne: status.DELETE } });
            if (!subscriptionResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            var result = await updateSubscription({ _id: subscriptionResult._id }, validatedBody);
    ...
    async listNFT(req, res, next) {
        ...
        try {
            ...
            let dataResults = await nftListWithAggregatePipelineForAll(validatedBody, userResult._id);
            return res.json(new response(dataResults, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
    ...
    async viewOrder(req, res, next) {
        ...
        try {
            ...
            return res.json(new response(orderResult, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
        }
    ...
    async deleteOrder(req, res, next) {
        ...
        try {
            ...
            let cancelResData = await openMarketPlaceContract.methods.cancelOrder(NFTTokenContract, orderResult.tokenId).encodeABI();
            let transactionResult = await transaction(openMarketPlaceContract, cancelResData, adminPrivateKey);
            if (transactionResult.status === true) {
                var result = await updateOrder({ _id: orderResult._id }, { status: status.DELETE });
                await updateBid({ orderId: orderResult._id }, { status: status.DELETE });
                return res.json(new response(result, responseMessage.DETAILS_FETCHED));
            } else {
    ...
    async stopAuction(req, res, next) {
        ...
        try {
            ...
            var result = await updateOrder({ _id: orderResult._id }, { status: "STOPPED" });
    ...
    async totalAdminBalance(req, res, next) {
        try {
            ...
            const eth = async () => {
                return await axios.get(`${blockchainUrl}/ethGetBalance/${adminResult.ethAccount.address}`);
            }
            const mass = async () => {
                return await bnbHelper.getMassBalance(adminResult.ethAccount.address);
            }
            const bnb = async () => {
                return await axios.get(`${blockchainUrl}/bnbGetBalance/${adminResult.ethAccount.address}`);
            }
            const usdt = async () => {
    ...
    async getAdminTotalEarnings(req, res, next) {
        try {
            ...
            var result = await findEarning({ userId: adminResult._id, status: status.ACTIVE });
            return res.json(new response(result, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
    ...
    async getTotalUserFunds(req, res, next) {
        try {
            ...
            var aggregate = [{
                $match: {
                    userType: { $in: [userType.CREATOR, userType.USER] }, status: { $ne: status.DELETE }
                }
            },
            {
                $group: {
    ...
    async viewModerator(req, res, next) {
        ...
        try {
            ...
            var moderatorResult = await findModerator({ _id: _id, status: { $ne: status.DELETE } });
            if (!moderatorResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(moderatorResult, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
        }
    ...
    async editModerator(req, res, next) {
        ...
        try {
            ...
            var moderatorResult = await findModerator({ _id: validatedBody._id, status: { $ne: status.DELETE } });
            if (!moderatorResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            var result = await updateModerator({ _id: moderatorResult._id }, validatedBody);
            return res.json(new response(result, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
    ...
    async viewReport(req, res, next) {
        ...
        try {
            ...
            var reportResult = await findReport({ _id: _id, status: { $ne: status.DELETE } });
            if (!reportResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(reportResult, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
        }
    ...
    async editReport(req, res, next) {
        ...
        try {
            ...
            var reportResult = await findReport({ _id: validatedBody._id, status: { $ne: status.DELETE } });
            if (!reportResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
    ...
    async deleteReport(req, res, next) {
        ...
        try {
            const { _id } = await Joi.validate(req.query, validationSchema);
            let reportResult = await findReport({ _id: _id, status: { $ne: status.DELETE } });
            if (!reportResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            let result = await updateReport({ _id: reportResult._id }, { status: status.DELETE });
    ...
    async listReport(req, res, next) {
        try {
            ...
            let dataResults = await reportList({ status: { $ne: status.DELETE }, actionApply: false, reportStatus: "PENDING" }, req.query.page, req.query.limit);
            return res.json(new response(dataResults, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
    ...
    async blockUser(req, res, next) {
        ...
        try {
            ...
            let tillValid = new Date(new Date().getTime() + parseFloat(validatedBody.time.split(" ")[0]) * 60 * 60 * 1000).toISOString();
            ...
            await updateReport({ _id: validatedBody.reportId }, { actionApply: true, reportStatus: "RESOLVED" });
            let obj = {
                userId: userCheck._id,
    ...
    async sendWarningMessage(req, res, next) {
        ...
        try {
            ...
            await updateReport({ _id: reportId }, { actionApply: true, reportStatus: "RESOLVED" });
    ...
    */
    ...
    async viewFee(req, res, next) {
        ...
        try {
            ...
            var feeResult = await findFee({ _id: _id, status: { $ne: status.DELETE } });
            if (!feeResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
    ...
    async editFee(req, res, next) {
        ...
        try {
            ...
            var feeResult = await findFee({ _id: validatedBody._id, status: { $ne: status.DELETE } });
            if (!feeResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
    ...
    async deleteFee(req, res, next) {
        ...
        try {
            const { _id } = await Joi.validate(req.query, validationSchema);
            let feeResult = await findFee({ _id: _id, status: { $ne: status.DELETE } });
            if (!feeResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            let result = await updateFee({ _id: feeResult._id }, { status: status.DELETE });
            return res.json(new response(result, responseMessage.DETAILS_FETCHED));
        }
        catch (error) {
            return next(error);
    ...
    async listUser(req, res, next) {
        ...
        try {
            ...
            let dataResults = await userAllDetailsWithBundleCount(validatedBody, userResult._id);
            return res.json(new response({ finalData, dataResults }, responseMessage.DATA_FOUND));
        }
        catch (error) {
            return next(error);
        }
    ...
    async addPress(req, res, next) {
        ...
        try {
            const validatedBody = await Joi.validate(req.body, validationSchema);
            let admin = await findUser({ _id: req.userId, userType: { $in: [userType.ADMIN, userType.SUB_ADMIN] }, status: status.ACTIVE })
            if (!admin) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND)
            }
            var press = await findPress({ title: validatedBody.title, status: { $ne: status.DELETE } });
            if (press) {
                throw apiError.conflict(responseMessage.TITLE_EXIST);
            }
    ...
    async editPress(req, res, next) {
        ...
        try {
            ...
            var press = await findPress({ _id: req.query._id, status: { $ne: status.DELETE } });
            if (!press) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            else {
                ...
                var update = await updatePress({ _id: press._id }, { $set: validatedBody });
    ...
    async viewPress(req, res, next) {
        ...
        try {
            ...
            var feeResult = await findPress({ _id: validatedBody._id, status: { $ne: status.DELETE } });
            if (!feeResult) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(feeResult, responseMessage.DETAILS_FETCHED));
    ...
    async deletePress(req, res, next) {
        ...
        try {
            ...
            let result = await findPress({ _id: validatedBody._id, status: { $ne: status.DELETE } });
            if (!result) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            let update = await updatePress({ _id: result._id }, { $set: { status: status.DELETE } });
    ...
    async activeDeactivePress(req, res, next) {
        ...
        try {
            ...
            let adminResult = await findUser({ _id: req.userId, userType: { $in: [userType.ADMIN, userType.SUB_ADMIN] } })
            if (!adminResult) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND)
            }
            ...
            if (pressInfo.status == status.ACTIVE) {
                let blockRes = await updatePress({ _id: pressInfo._id }, { $set: { status: status.BLOCK } })
                return res.json(new response(blockRes, "Deactivate press details ."))
            }
            if (pressInfo.status == status.BLOCK) {
                let activeRes = await updatePress({ _id: pressInfo._id }, { $set: { status: status.ACTIVE } })
    ...
    async addPartner(req, res, next) {
        ...
        try {
            ...
            var press = await findPartner({ name: validatedBody.name, status: { $ne: status.DELETE } });
            if (press) {
    ...
    async editPartner(req, res, next) {
        ...
        try {
            ...
            var press = await findPartner({ _id: req.query._id, status: { $ne: status.DELETE } });
            if (!press) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            else {
                ...
                var update = await updatePartner({ _id: press._id }, { $set: validatedBody });
                return res.json(new response(update, responseMessage.UPDATE_SUCCESS));
            }
        }
        catch (error) {
    ...
    async viewPartner(req, res, next) {
        ...
        try {
            var validatedBody = await Joi.validate(req.query, validationSchema);
            var result = await findPartner({ _id: validatedBody._id, status: { $ne: status.DELETE } });
            if (!result) {
                throw apiError.conflict(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(result, responseMessage.DETAILS_FETCHED));
    ...
    async deletePartner(req, res, next) {
        ...
        try {
            ...
            let result = await findPartner({ _id: validatedBody._id, status: { $ne: status.DELETE } });
            if (!result) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            let update = await updatePartner({ _id: result._id }, { $set: { status: status.DELETE } });
            return res.json(new response(update, responseMessage.DELETE_SUCCESS));
        }
        catch (error) {
            return next(error);
    ...
    async activeDeactivePartner(req, res, next) {
        ...
        try {
            ...
            if (partnerInfo.status == status.ACTIVE) {
                let blockRes = await updatePartner({ _id: partnerInfo._id }, { $set: { status: status.BLOCK } })
                return res.json(new response(blockRes, "Deactivate partner successfully ."))
            } if (partnerInfo.status == status.BLOCK) {
                let activeRes = await updatePartner({ _id: partnerInfo._id }, { $set: { status: status.ACTIVE } })
                return res.json(new response(activeRes, "Partner activated successfully ."))
            }
        } catch (error) {
            return next(error)
        }
    ...
    async addAdvertisement(req, res, next) {
        ...
        try {
            ...
            let admin = await findUser({ _id: req.userId, userType: { $in: [userType.ADMIN, userType.SUB_ADMIN] } })
            if (!admin) {
                throw apiError.invalid(responseMessage.USER_NOT_FOUND)
            }
            let find = await findAdvertisement({ title: validBody.title, status: status.ACTIVE })
            if (find) {
    ...
    async viewAdvertisement(req, res, next) {
        ...
        try {
            ...
            let find = await findAdvertisement({ _id: _id, status: { $ne: status.DELETE } })
            if (!find) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
    ...
    async editAdvertisement(req, res, next) {
        ...
        try {
            ...
            let data = await findAdvertisement({ _id: req.query._id, status: { $ne: status.DELETE } })
            if (!data) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            ...
            let update = await updateAdvertisementById({ _id: data._id }, { $set: validBody })
            return res.json(new response(update, responseMessage.UPDATE_SUCCESS))
        } catch (error) {
            return next(error)
        }
    ...
    async removeAdvertisement(req, res, next) {
        ...
        try {
            ...
            let data = await findAdvertisement({ _id: _id, status: { $ne: status.DELETE } })
            if (!data) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            let result = await updateAdvertisementById({ _id: data._id }, { $set: { status: status.DELETE } })
    ...
    async activeDeactiveAdvertisement(req, res, next) {
        ...
        try {
            ...
            if (!advertisement) {
                ...
                let set = await updateAdvertisementById({ _id: advertisement._id }, { $set: { status: status.BLOCK } })
                return res.json(new response(set, "Advertisement deactivated successfully ."))
            }
            else {
                let set = await updateAdvertisementById({ _id: advertisement._id }, { $set: { status: status.ACTIVE } })
                return res.json(new response(set, "Advertisement activated successfully ."))
            }
        } catch (error) {
            return next(error)
        }
    ...
    async viewSocial(req, res, next) {
        ...
        try {
            ...
            let find = await findSocial({ title: title, status: { $ne: status.DELETE } })
            if (!find) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
    ...
    async editSocial(req, res, next) {
        ...
        try {
            ...
            let data = await findSocial({ _id: socialId, status: { $ne: status.DELETE } })
            if (!data) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            let update = await updateSocialById({ _id: data._id }, { $set: { link: link } })
            return res.json(new response(update, responseMessage.UPDATE_SUCCESS))
        } catch (error) {
            return next(error)
        }
    ...
    async addLogo(req, res, next) {
        ...
        try {
            ...
            if (!admin) {
                ...
                let find = await findLogo({ logoTitle: validatedBody.logoTitle, status: status.ACTIVE })
                if (find) {
                    throw apiError.conflict(responseMessage.ALREADY_EXITS)
                }
                let obj = {
    ...
    async viewLogo(req, res, next) {
        ...
        try {
            const validatedBody = await Joi.validate(req.query, validateSchema)
            let find = await findLogo({ _id: validatedBody._id, status: { $ne: status.DELETE } })
            if (!find) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
    ...
    async editLogo(req, res, next) {
        ...
        try {
            ...
            let adminRes = await findUser({ _id: req.userId, userType: { $in: [userType.ADMIN, userType.SUB_ADMIN] } })
            if (!adminRes) {
                throw apiError.invalid(responseMessage.UNAUTHORIZED)
            }
            let data = await findLogo({ _id: req.query._id, status: { $ne: status.DELETE } })
            if (!data) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            else {
                ...
                let update = await updateLogoById({ _id: data._id }, { $set: validatedBody })
    ...
    async deleteLogo(req, res, next) {
        ...
        try {
            ...
            let result = await findLogo({ _id: validatedBody._id, status: { $ne: status.DELETE } })
            if (!result) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            let updateRes = await updateLogoById({ _id: result._id }, { status: status.DELETE })
    ...
    async subscriptionListOfParticular(req, res, next) {
        try {
            ...
            let user = await findUser({ _id: req.query.userId, status: status.ACTIVE, userType: { $ne: userType.ADMIN } })
            if (!user) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND)
            }
            else {
    ...
    async viewSubscriptionOfParticular(req, res, next) {
        try {
            ...
            let user = await findTransaction({ userId: req.query.userId, status: status.ACTIVE, userType: { $ne: userType.ADMIN } })
            if (!user) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND)
            }
            else {
    ...
    async addSubAdmin(req, res, next) {
        ...
        try {
            const { email, name, password, permissions } = await Joi.validate(req.body, validationSchema);
            var userResult = await findUser({ _id: req.userId, userType: userType.ADMIN })
            if (!userResult) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND);
            }
            var emailResult = await findUser({ email: email, status: { $ne: status.DELETE } });
            if (emailResult) {
                throw apiError.conflict(responseMessage.EMAIL_EXIST);
            }
            var obj = {
    ...
    async editSubAdmin(req, res, next) {
        ...
        try {
            ...
            var subAdminResult = await findUser({ _id: validatedBody._id, status: status.ACTIVE });
            if (!subAdminResult) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND);
            }
            if (validatedBody.email) {
                emailResult = await findUser({ email: validatedBody.email, status: { $ne: status.DELETE }, _id: { $ne: subAdminResult._id } });
                if (emailResult) {
                    throw apiError.conflict(responseMessage.EMAIL_EXIST);
                }
            }
    ...
    async blockUnblockSubAdmin(req, res, next) {
        ...
        try {
            ...
            return res.json(new response(result, validatedBody.status == status.BLOCK ? responseMessage.BLOCKED : responseMessage.UNBLOCKED));
        }
        catch (error) {
            return next(error)
        }
    ...
    async getReferralSetting(req, res, next) {
        try {
            var adminResult = await findUser({ _id: req.userId, userType: { $in: [userType.ADMIN, userType.SUB_ADMIN] } });
            if (!adminResult) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND);
            }
    ...
    async updateReferralSetting(req, res, next) {
        ...
        try {
            const { _id, referralAmount, refereeAmount } = await Joi.validate(req.body, validationSchema);
            var adminResult = await findUser({ _id: req.userId, userType: { $in: [userType.ADMIN, userType.SUB_ADMIN] } });
            if (!adminResult) {
                throw apiError.notFound(responseMessage.USER_NOT_FOUND);
            }
            var result = await updateReferral({ _id: _id }, { $set: { referralAmount: referralAmount, refereeAmount: refereeAmount } });
            return res.json(new response(result, responseMessage.UPDATE_SUCCESS));
    ...
    async addBanner(req, res, next) {
        ...
        try {
            ...
            let bannerResult = await findBanner({ title: validBody.title, status: status.ACTIVE })
            if (bannerResult) {
                throw apiError.conflict(responseMessage.ALREADY_EXITS)
            }
            let obj = {
    ...
    async viewBanner(req, res, next) {
        ...
        try {
            const { _id } = await Joi.validate(req.query, validSchema)
            let result = await findBanner({ _id: _id, status: { $ne: status.DELETE } })
            if (!result) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            return res.json(new response(result, responseMessage.DATA_FOUND))
    ...
    async editBanner(req, res, next) {
        ...
        try {
            ...
            let bannerResult = await findBanner({ _id: validBody._id, status: { $ne: status.DELETE } })
            if (!bannerResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            if (req.files.length != 0) {
            ...
            let result = await updateBanner({ _id: bannerResult._id }, { $set: validBody })
            return res.json(new response(result, responseMessage.UPDATE_SUCCESS))
        } catch (error) {
            return next(error)
        }
    ...
    async deleteBanner(req, res, next) {
        ...
        try {
            ...
            let bannerResult = await findBanner({ _id: _id, status: { $ne: status.DELETE } })
            if (!bannerResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            let result = await updateBanner({ _id: bannerResult._id }, { $set: { status: status.DELETE } })
            return res.json(new response(result, responseMessage.DELETE_SUCCESS))
        } catch (error) {
            return next(error)
        }
    ...
    async changeBannerStatus(req, res, next) {
        ...
        try {
            ...
            let bannerResult = await findBanner({ _id: validatedBody._id, status: { $ne: status.DELETE } })
            if (!bannerResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            var result = await updateBanner({ _id: bannerResult._id }, { status: validatedBody.status })
            return res.json(new response(result, validatedBody.status == status.BLOCK ? responseMessage.BLOCKED : responseMessage.UNBLOCKED));
        } catch (error) {
            return next(error)
        }
    ...
    async addVideo(req, res, next) {
        ...
        try {
            ...
            let videoResult = await findVideo({ title: validBody.title, status: status.ACTIVE })
            if (videoResult) {
                throw apiError.conflict(responseMessage.ALREADY_EXITS)
            }
            let obj = {
    ...
    async viewVideo(req, res, next) {
        ...
        try {
            const { _id } = await Joi.validate(req.query, validSchema)
            let result = await findVideo({ _id: _id, status: { $ne: status.DELETE } })
            if (!result) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            return res.json(new response(result, responseMessage.DATA_FOUND))
    ...
    async editVideo(req, res, next) {
        ...
        try {
            ...
            let videoResult = await findVideo({ _id: validBody._id, status: { $ne: status.DELETE } })
            if (!videoResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            if (req.files) {
            ...
            let result = await updateVideo({ _id: videoResult._id }, { $set: validBody })
            return res.json(new response(result, responseMessage.UPDATE_SUCCESS))
        } catch (error) {
            return next(error)
        }
    ...
    async deleteVideo(req, res, next) {
        ...
        try {
            ...
            let videoResult = await findVideo({ _id: _id, status: { $ne: status.DELETE } })
            if (!videoResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            let result = await updateVideo({ _id: videoResult._id }, { $set: { status: status.DELETE } })
            return res.json(new response(result, responseMessage.DELETE_SUCCESS))
        } catch (error) {
            return next(error)
        }
    ...
    async changeVideoStatus(req, res, next) {
        ...
        try {
            ...
            let videoResult = await findVideo({ _id: validatedBody._id, status: { $ne: status.DELETE } })
            if (!videoResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND)
            }
            var result = await updateVideo({ _id: videoResult._id }, { status: validatedBody.status })
            return res.json(new response(result, validatedBody.status == status.BLOCK ? responseMessage.BLOCKED : responseMessage.UNBLOCKED));
        } catch (error) {
            return next(error)
        }
