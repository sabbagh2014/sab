...
const { createSubscription, findSubscription, updateSubscription } = subscriptionServices;
...
let jobMain = new cronJob('*/10 * * * * *', async function () {
    var getBufferResult = await subscribeBufferList({ status: { $ne: status.DELETE }, transactionStatus: "PENDING" });
    if (getBufferResult.length == 0) {
        ...
        let userAddress = await findUser({ _id: bundleCheck.userId, status: { $ne: status.DELETE } });
        let subscriptionRes = await findSubscription({ userId: getBufferResult[0].userId, nftId: getBufferResult[0].nftId });
        ...
        if (userAddress.supporters.includes(getBufferResult[0].userId)) {
            ...
            if (getBufferResult[0].coin === "ETH") {
                updateQuery.$inc = { subscriberCount: 1, ethBalance: Number(donationAmount) };
            }
            if (getBufferResult[0].coin === "BNB") {
                updateQuery.$inc = { subscriberCount: 1, bnbBalace: Number(donationAmount) };
            }
            if (getBufferResult[0].coin === "USDT") {
                updateQuery.$inc = { subscriberCount: 1, usdtBalance: Number(donationAmount) };
            }
            if (getBufferResult[0].coin === "MASS") {
                updateQuery.$inc = { subscriberCount: 1, massBalance: Number(donationAmount) };
            }
            if (getBufferResult[0].coin === "WBTC") {
                updateQuery.$inc = { subscriberCount: 1, btcBalance: Number(donationAmount) };
            }
        }
        notificationObj = {
        ...
        if (subscriptionRes) {
            if (subscriptionRes.subscriptionStatus !== status.EXPIRED) {
                ...
                myDate.setDate(myDate.getDate() + parseInt(getBufferResult[0].duration));
                getBufferResult[0].validTillDate = myDate.toISOString();
            }
            await updateSubscription({ _id: subscriptionRes._id }, { subscriptionStatus: status.ACTIVE, validTillDate: getBufferResult[0].validTillDate });
            if (getBufferResult[0].coin === "ETH" || getBufferResult[0].coin === "BNB") {
                ...
                try {
                    ...
                    await updateNft({ _id: bundleCheck._id }, { $addToSet: { subscribers: getBufferResult[0].userId }, $inc: { subscriberCount: 1 } });
                    await updateUser({ _id: bundleCheck.userId }, updateQuery);
                    await updateUser({ _id: getBufferResult[0].userId }, { $addToSet: { subscribeNft: bundleCheck._id } });
                    addUserIntoFeed(bundleCheck._id, getBufferResult[0].userId);
                    await updateSubscribeBuffer({ _id: getBufferResult[0]._id }, { transactionStatus: "SUCCESS", transactionHash: transactionHash.data.Hash });
                    console.log("Successfully completed transaction for id  ", getBufferResult[0]._id)
                    jobMain.stop();
                    jobMain.start();
                } catch (error) {
                    jobMain.stop();
            ...
            if (getBufferResult[0].coin === "ETH" || getBufferResult[0].coin === "BNB") {
                ...
                try {
                    transactionHash = await axios.post(`${blockchainUrl}/distributeTokens`, transObj);
                    console.log("151== MASS && WBTC && BNB ====>>", transactionHash.data.Hash)
                    sendNotification(getBufferResult[0].userId, transactionHash.data.Hash)
                    transactionObj.transactionHash = transactionHash.data.Hash;
                    await createTransaction(transactionObj)
                    await createNotification(notificationObj);
