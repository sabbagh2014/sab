...
new cronJob('*/50 * * * * *', async function () {
    var subscriberRes = await subscriptionList({ status: { $ne: status.DELETE }, validTillDate: { $lt: new Date(new Date().setHours(new Date().getHours() - 1)).toISOString() }, subscriptionStatus: status.ACTIVE });
    if (subscriberRes.length == 0) {
        console.log("No records found for subscription alert.")
    } else {
        for (let i = 0; i < subscriberRes.length; i++) {
            const updateRes = await updateSubscription({ _id: subscriberRes[i]._id }, { $set: { isAlert: true } });
