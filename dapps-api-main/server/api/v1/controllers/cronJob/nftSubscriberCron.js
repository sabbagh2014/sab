...
new cronJob('*/50 * * * * *', async function () {
    var subscriberRes = await subscriptionList({ status: { $ne: status.DELETE }, validTillDate: { $lt: new Date().toISOString() }, subscriptionStatus: status.ACTIVE });
    if (subscriberRes.length == 0) {
        ...
        for (let i = 0; i < subscriberRes.length; i++) {
            ...
            const updateRes = await updateSubscription({ _id: subscriberRes[i]._id }, { $set: { subscriptionStatus: status.EXPIRED } });
            await updateUser({ _id: subscriberRes[i].userId }, { $pull: { subscribeNft: subscriberRes[i].nftId._id } });
            sendNotification(subscriberRes[i].userId, subscriberRes[i].nftId.bundleName);
            if (updateRes) {
